...
BASE_URL = "https://www.mydesi2.fit/"
CANONICAL_HOST = "www.mydesi2.fit"
...
def can_handle(host: str) -> bool:
    ...
    return h in _SUPPORTED_HOSTS or h.endswith(".mydesi2.fit")
...
def _microdata(soup: BeautifulSoup, itemprop: str) -> Optional[str]:
...
def _normalize_duration(value: Any) -> Optional[str]:
    ...
    if isinstance(value, str):
        v = value.strip()
        match = re.fullmatch(r"P(?:(\d+)D)?T(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?", v)
        if match:
...
def _clean_title(title: str | None) -> Optional[str]:
    ...
    for suffix in (
        ...
        " - MyDesi",
        " | MyDesi",
        " - MyDesi HD - Free Indian Sex, Desi Porn Videos & Leaked MMS Online",
    ):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if _normalize_host(parsed.netloc) not in _SUPPORTED_HOSTS:
        return None
    if any(x in parsed.path.lower() for x in ("/wp-content/", "/wp-json/", "/tag/", "/tags/", "/category/", "/categories/", "/page/", "/author/", "/feed/", "/contact", "/privacy", "/dmca", "/18-u-s-c-2257", "/about-us", "/content-complaint")):
        return None
    if parsed.query:
        return None
...
def _extract_streams(soup: BeautifulSoup, html: str, extra_urls: Optional[list[str]] = None) -> dict[str, Any]:
...
def parse_video_page(html: str, url: str) -> dict[str, Any]:
    ...
    upload_date = _first_non_empty(
        ...
        _microdata(soup, "uploadDate"),
    )
    category = _meta(soup, prop="article:section")

    content_url = _microdata(soup, "contentURL")
    ...
    duration = _normalize_duration(_microdata(soup, "duration"))
    uploader = None
    views = None
    ...
    if not duration:
        ...
        if dm and dm.group(0) not in ("0:00", "0:0"):
            duration = dm.group(0)
    if not views:
        views = _extract_views_text(text_blob)

    tags = list(dict.fromkeys([t for t in tags if t]))
    video = _extract_streams(soup, html, extra_urls=[content_url] if content_url else None)
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    for a in soup.select("a[href]"):
        ...
        dur_el = container.select_one(".duration") if container else None
        if dur_el:
            dt = dur_el.get_text(" ", strip=True)
            if dt:
                duration = dt
        if not duration:
        ...
        views_el = container.select_one(".views-number") if container else None
        if views_el:
            vt = views_el.get_text(" ", strip=True)
            if vt:
                views = _clean_views_text(vt)
        if not views:
