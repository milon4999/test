...
BASE_SITE = "https://hello.porn/"
SITE_ALIASES = frozenset({"hello.porn", "www.hello.porn", "static.hello.porn"})
...
_VIDEO_HREF_RE = re.compile(
    r"(?:https?://(?:www\.)?hello\.porn)?/videos/(?P<id>\d+)/?",
    re.IGNORECASE,
)
_EMBED_HREF_RE = re.compile(
    r"(?:https?://(?:www\.)?hello\.porn)?/embed/(?P<id>\d+)/?",
    re.IGNORECASE,
)
_GET_FILE_RE = re.compile(
    r"https?://(?:www\.)?hello\.porn/get_file/[^\s\"'<>]+",
    re.IGNORECASE,
)
...
def can_handle(host: str) -> bool:
    ...
    if h.startswith("www."):
        h = h[4:]
    return h in SITE_ALIASES or h.endswith(".hello.porn")
...
def _normalize_video_href(href: str, *, base: str = BASE_SITE) -> Optional[str]:
    ...
    if host.startswith("www."):
        host = host[4:]
    if host and host not in {"hello.porn", "www.hello.porn"}:
        return None
    m = _VIDEO_HREF_RE.search(parsed.path or "")
    if not m:
        return None
    return urlunparse(("https", "hello.porn", f"/videos/{m.group('id')}/", "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    host = parsed.netloc or "hello.porn"
...
def _streams_from_page(html: str, soup: BeautifulSoup) -> dict[str, Any]:
    ...
    for stream in streams:
        key = stream["quality"]
        if key not in best or (key == "auto" and best[key]["url"] == stream["url"]):
            if key not in best:
                best[key] = stream
...
async def _resolve_video_streams(video: dict[str, Any], *, referer: str) -> None:
    ...
    # Drop lower-quality get_file copies once an HLS master is resolved.
    if seen_resolved:
        streams[:] = [
            s
            for s in streams
...
def _parse_card(block: Any, *, base: str, exclude_url: str | None = None) -> Optional[dict[str, Any]]:
    ...
    title = _first_non_empty(
        ...
        _text(block.select_one("h4")),
    ) or "Unknown Video"
    duration = _normalize_duration(_text(block.select_one(".duration_item")))
    return {
        "url": abs_url,
        "title": title.strip(),
        "thumbnail_url": thumb,
...
def _parse_cards(
    ...
    *,
    ...
    seen: set[str] = set()
    for block in soup.select(".items-videos .item, .related-videos .item"):
...
def parse_page(html: str, url: str) -> dict[str, Any]:
    ...
    if title:
        for suffix in (" | HELLO.PORN", " - HELLO.PORN"):
    ...
    for obj in _parse_json_ld(soup):
        ...
        if isinstance(actor, list) and actor:
            uploader = str(actor[0]).strip() or None
