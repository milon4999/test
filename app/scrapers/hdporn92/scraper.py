...
BASE_SITE = "https://hdporn92.com/"
SITE_HOST = "hdporn92.com"
SITE_ALIASES = frozenset({"hdporn92.com", "www.hdporn92.com"})
...
_POST_PAGE_RE = re.compile(
    r"^https?://(?:www\.)?hdporn92\.com/(?P<slug>[a-z0-9][a-z0-9-]*)/?$",
    re.IGNORECASE,
)
_MP4_RE = re.compile(
    r"https?://[^\s\"'<>]+\.mp4(?:\?[^\s\"'<>]*)?",
...
def can_handle(host: str) -> bool:
    ...
    if h.startswith("www."):
        h = h[4:]
    return h in SITE_ALIASES or h.endswith(".hdporn92.com")
    ...
    for pat, fmt in ((_M3U8_RE, "hls"), (_MP4_RE, "mp4")):
        for raw in pat.findall(html_norm):
            ...
            if "/wp-content/" in low:
                continue
            seen.add(url)
            streams.append({"url": url, "quality": "adaptive", "format": fmt})
...
def _embed_streams(soup: BeautifulSoup) -> dict[str, Any]:
    """Collect player iframes from .responsive-player (retrotube player wrapper)."""
    embed_urls: list[str] = []
...
def _parse_list_items(soup: BeautifulSoup, *, limit: int) -> list[dict[str, Any]]:
    ...
    for block in soup.select("article.thumb-block, .thumb-block.video-preview-item"):
        ...
        # Views: eye icon + span ("5K" — kept verbatim)
        views = None
...
def parse_video_page(html: str, url: str, *, video: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    views_el = soup.select_one("#video-views span")
    if views_el:
        views = views_el.get_text(strip=True) or None

    # Duration: none exposed; upload date from itemprop or Rank Math meta
    upload_date = _first_non_empty(
        _itemprop(soup, "uploadDate"),
        _meta(soup, prop="article:published_time"),
    )

    # Tags: /category/ links inside the article .tags-list; actors in #video-actors
    tags: list[str] = []
    ...
    # Description: og:description carries the full story text
    description = _first_non_empty(
        _meta(soup, prop="og:description"), _meta(soup, name="description")
    )
    if description and description.endswith("[&hellip;]"):
        description = description[: -len("[&hellip;]")].strip()
...
async def scrape(url: str) -> dict[str, Any]:
    ...
    if not canon:
        raise ValueError(f"Unsupported HDporn92 URL: {url}")
