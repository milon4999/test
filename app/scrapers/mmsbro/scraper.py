...
# Live content is served on desifile.org. Legacy hosts rewrite to the canonical domain for fetches.
CANONICAL_HOST = "desifile.org"
...
def _normalize_media_url(src: str, base: str = "https://desifile.org/") -> Optional[str]:
    u = (src or "").strip()
    if not u:
        return None
    if u.startswith("//"):
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    rh = _registry_host(parsed.netloc)
    netloc = CANONICAL_HOST if (not parsed.netloc or rh in _REGISTRY_HOSTS) else parsed.netloc
    path = parsed.path or "/"
