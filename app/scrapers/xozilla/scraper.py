...
BASE_SITE = "https://www.xozilla.com/"
SITE_ALIASES = frozenset({"xozilla.com", "www.xozilla.com", "i.xozilla.com", "vcdn.xozilla.com"})
...
_VIDEO_HREF_RE = re.compile(
    r"(?:https?://(?:www\.)?xozilla\.com)?/videos/(?P<id>\d+)(?:/(?P<slug>[^/?#]+))?/?",
    re.IGNORECASE,
)
_EMBED_HREF_RE = re.compile(
    r"(?:https?://(?:www\.)?xozilla\.com)?/embed/(?P<id>\d+)/?",
    re.IGNORECASE,
)
...
_GET_FILE_RE = re.compile(r"https?://(?:www\.)?xozilla\.com/get_file/[^\s\"'<>]+", re.IGNORECASE)
...
def can_handle(host: str) -> bool:
    ...
    if h.startswith("www."):
        h = h[4:]
    return h in SITE_ALIASES or h.endswith(".xozilla.com")
...
def _normalize_video_href(href: str, *, base: str = BASE_SITE) -> Optional[str]:
    ...
    if host.startswith("www."):
        host = host[4:]
    if host and host not in {"xozilla.com", "www.xozilla.com"}:
        return None
    ...
    return urlunparse(("https", "www.xozilla.com", path, "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    host = parsed.netloc or "www.xozilla.com"
...
def _parse_cards(
    ...
    *,
    ...
    seen: set[str] = set()
    for block in soup.select("a.item[href*='/videos/'], .list-videos a.item"):
...
def parse_page(html: str, url: str) -> dict[str, Any]:
    ...
    if title:
        for suffix in (" / Xozilla.com", " | Xozilla.com", " - Xozilla"):
