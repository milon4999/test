...
BASE_SITE = "https://homo.xxx/"
SITE_ALIASES = frozenset({"homo.xxx", "www.homo.xxx", "static.homo.xxx"})
...
_VIDEO_HREF_RE = re.compile(
    r"(?:https?://(?:www\.)?homo\.xxx)?/videos/(?P<id>\d+)/?",
    re.IGNORECASE,
)
_EMBED_HREF_RE = re.compile(
    r"(?:https?://(?:www\.)?homo\.xxx)?/embed/(?P<id>\d+)/?",
    re.IGNORECASE,
)
_GET_FILE_RE = re.compile(
    r"https?://(?:www\.)?homo\.xxx/get_file/[^\s\"'<>]+",
    re.IGNORECASE,
)
...
def can_handle(host: str) -> bool:
    ...
    if h.startswith("www."):
        h = h[4:]
    return h in SITE_ALIASES or h.endswith(".homo.xxx")
...
def _normalize_video_href(href: str, *, base: str = BASE_SITE) -> Optional[str]:
    ...
    if host.startswith("www."):
        host = host[4:]
    if host and host not in {"homo.xxx", "www.homo.xxx"}:
        return None
    m = _VIDEO_HREF_RE.search(parsed.path or "")
    if not m:
        return None
    return urlunparse(("https", "homo.xxx", f"/videos/{m.group('id')}/", "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    host = parsed.netloc or "homo.xxx"
...
def parse_page(html: str, url: str) -> dict[str, Any]:
    ...
    if title:
        for suffix in (" | HOMO.XXX", " - HOMO.XXX"):
    ...
    tags = list(dict.fromkeys([t for t in tags if t.lower() not in {"porn", "xxx", "hd", "gay"}]))
    if tags:
        category = tags[0]
