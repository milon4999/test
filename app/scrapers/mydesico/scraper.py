...
BASE_URL = "https://mydesi.com.co/"
CANONICAL_HOST = "mydesi.com.co"
...
def can_handle(host: str) -> bool:
    ...
    return h in _SUPPORTED_HOSTS or h.endswith(".mydesi.com.co")
...
def _clean_title(title: str | None) -> Optional[str]:
    ...
    for suffix in (
        " - MyDesi",
        " | MyDesi",
        " - MyDesi | HD Desi Sex, Bhabhi MMS Leaks & Nude Clips",
        " - MyDesi | Indian Porn",
        " | MyDesi | Indian Porn",
    ):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if len(segments) != 2 or segments[0].lower() != "videos":
        return None
    slug = segments[1]
    return urlunparse(("https", CANONICAL_HOST, f"/videos/{slug}", "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    m = re.match(r"^(/videos/filter/[^/]+/[^/]+)/\d+$", clean_path)
    if m:
        return urlunparse((scheme, netloc, f"{m.group(1)}/{page}", "", urlencode(query_items), ""))

    if clean_path == "/videos":
        return urlunparse((scheme, netloc, f"/videos/filter/recent/all/{page}", "", urlencode(query_items), ""))

    m = re.match(r"^(/categories/[^/]+)/\d+$", clean_path)
    if m:
        return urlunparse((scheme, netloc, f"{m.group(1)}/{page}", "", urlencode(query_items), ""))

    m = re.match(r"^(/categories/[^/]+)$", clean_path)
    if m:
        return urlunparse((scheme, netloc, f"{m.group(1)}/{page}", "", urlencode(query_items), ""))
