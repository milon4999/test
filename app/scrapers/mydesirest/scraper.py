...
BASE_URL = "https://mydesi.rest/"
CANONICAL_HOST = "mydesi.rest"
...
# Thumbnails / media are served from a separate CDN host under tdn.mydesi.rest
_CDN_HOST = "tdn.mydesi.rest"
...
def can_handle(host: str) -> bool:
    ...
    return h in _SUPPORTED_HOSTS or h.endswith(".mydesi.rest")
...
def _clean_title(title: str | None) -> Optional[str]:
    ...
    t = title.strip()
    for suffix in (
        " - Mydesi | Watch Desi Latest Sex Videos and MMS",
        " | Mydesi | Watch Desi Latest Sex Videos and MMS",
        " - Mydesi | Desi Latest Sex Videos and MMS",
        " | Mydesi | Desi Latest Sex Videos and MMS",
        " - Mydesi",
        " | Mydesi",
        " - MyDesi",
...
def _extract_views_text(text: str | None) -> Optional[str]:
    if not text:
        return None
    m = re.search(r"\b(\d[\d,\.]*\s*[KMBkmb]?)\s*(?:views|view)\b", text, re.IGNORECASE)
    if m:
...
def _is_probable_playable_embed(src: str) -> bool:
    ...
    try:
        ...
        if _normalize_host(host) in _SUPPORTED_HOSTS or host == _CDN_HOST:
            return False
...
def _collect_player_iframes(soup: BeautifulSoup) -> list[Any]:
    for selector in (
        ...
        ".single-video-player iframe[src]",
        "#content iframe[src]",
    ):
...
def _extract_streams(soup: BeautifulSoup, html: str) -> dict[str, Any]:
    ...
    for a in soup.select("a[href]"):
        src = _normalize_media_url(a.get("href") or "")
        if not src or ".mp4" not in src.lower() or src in seen:
            continue
