...
async def _scrape_dispatch(url: str, host: str) -> dict[str, Any]:
    ...
    if blackporntube.can_handle(host): return await blackporntube.scrape(url)
    if sxyland.can_handle(host): return await sxyland.scrape(url)
    if camcaps.can_handle(host): return await camcaps.scrape(url)
    if koreanpornmovie.can_handle(host): return await koreanpornmovie.scrape(url)
    if fullporner.can_handle(host): return await fullporner.scrape(url)
    if superporn.can_handle(host): return await superporn.scrape(url)
    ...
    if joysporn.can_handle(host): return await joysporn.scrape(url)
    if fullxcinema.can_handle(host): return await fullxcinema.scrape(url)
    if filmadult.can_handle(host): return await filmadult.scrape(url)
    if pornhits.can_handle(host): return await pornhits.scrape(url)
    if pornmz.can_handle(host): return await pornmz.scrape(url)
    if fpoxxx.can_handle(host): return await fpoxxx.scrape(url)
    if watchporn.can_handle(host): return await watchporn.scrape(url)
    if helloporn.can_handle(host): return await helloporn.scrape(url)
    ...
    if xmilf.can_handle(host): return await xmilf.scrape(url)
    if hdzog.can_handle(host): return await hdzog.scrape(url)
    if analdin.can_handle(host): return await analdin.scrape(url)
    if nuvid.can_handle(host): return await nuvid.scrape(url)
    if p4455.can_handle(host): return await p4455.scrape(url)
    if mydesi2.can_handle(host): return await mydesi2.scrape(url)
    if mydesi10.can_handle(host): return await mydesi10.scrape(url)
    if mydesico.can_handle(host): return await mydesico.scrape(url)
    if mydesirest.can_handle(host): return await mydesirest.scrape(url)
    if mydesisbs.can_handle(host): return await mydesisbs.scrape(url)
    if viralchut.can_handle(host): return await viralchut.scrape(url)
    if viralmms.can_handle(host): return await viralmms.scrape(url)
    if xanimeporn.can_handle(host): return await xanimeporn.scrape(url)
    if vrsmash.can_handle(host): return await vrsmash.scrape(url)
    if porndish.can_handle(host): return await porndish.scrape(url)
    if mypornerleak.can_handle(host): return await mypornerleak.scrape(url)
    if momvids.can_handle(host): return await momvids.scrape(url)
...
async def _list_dispatch(base_url: str, host: str, page: int, limit: int) -> list[dict[str, Any]]:
    ...
    if blackporntube.can_handle(host): return await blackporntube.list_videos(base_url=base_url, page=page, limit=limit)
    if sxyland.can_handle(host): return await sxyland.list_videos(base_url=base_url, page=page, limit=limit)
    if camcaps.can_handle(host): return await camcaps.list_videos(base_url=base_url, page=page, limit=limit)
    if koreanpornmovie.can_handle(host): return await koreanpornmovie.list_videos(base_url=base_url, page=page, limit=limit)
    if fullporner.can_handle(host): return await fullporner.list_videos(base_url=base_url, page=page, limit=limit)
    if superporn.can_handle(host): return await superporn.list_videos(base_url=base_url, page=page, limit=limit)
    ...
    if joysporn.can_handle(host): return await joysporn.list_videos(base_url=base_url, page=page, limit=limit)
    if fullxcinema.can_handle(host): return await fullxcinema.list_videos(base_url=base_url, page=page, limit=limit)
    if filmadult.can_handle(host): return await filmadult.list_videos(base_url=base_url, page=page, limit=limit)
    if pornhits.can_handle(host): return await pornhits.list_videos(base_url=base_url, page=page, limit=limit)
    if pornmz.can_handle(host): return await pornmz.list_videos(base_url=base_url, page=page, limit=limit)
    if fpoxxx.can_handle(host): return await fpoxxx.list_videos(base_url=base_url, page=page, limit=limit)
    if watchporn.can_handle(host): return await watchporn.list_videos(base_url=base_url, page=page, limit=limit)
    if helloporn.can_handle(host): return await helloporn.list_videos(base_url=base_url, page=page, limit=limit)
    ...
    if xmilf.can_handle(host): return await xmilf.list_videos(base_url=base_url, page=page, limit=limit)
    if hdzog.can_handle(host): return await hdzog.list_videos(base_url=base_url, page=page, limit=limit)
    if analdin.can_handle(host): return await analdin.list_videos(base_url=base_url, page=page, limit=limit)
    if nuvid.can_handle(host): return await nuvid.list_videos(base_url=base_url, page=page, limit=limit)
    if p4455.can_handle(host): return await p4455.list_videos(base_url=base_url, page=page, limit=limit)
    if mydesi2.can_handle(host): return await mydesi2.list_videos(base_url=base_url, page=page, limit=limit)
    if mydesi10.can_handle(host): return await mydesi10.list_videos(base_url=base_url, page=page, limit=limit)
    if mydesico.can_handle(host): return await mydesico.list_videos(base_url=base_url, page=page, limit=limit)
    if mydesirest.can_handle(host): return await mydesirest.list_videos(base_url=base_url, page=page, limit=limit)
    if mydesisbs.can_handle(host): return await mydesisbs.list_videos(base_url=base_url, page=page, limit=limit)
    if viralchut.can_handle(host): return await viralchut.list_videos(base_url=base_url, page=page, limit=limit)
    if viralmms.can_handle(host): return await viralmms.list_videos(base_url=base_url, page=page, limit=limit)
    if xanimeporn.can_handle(host): return await xanimeporn.list_videos(base_url=base_url, page=page, limit=limit)
    if vrsmash.can_handle(host): return await vrsmash.list_videos(base_url=base_url, page=page, limit=limit)
    if porndish.can_handle(host): return await porndish.list_videos(base_url=base_url, page=page, limit=limit)
    if mypornerleak.can_handle(host): return await mypornerleak.list_videos(base_url=base_url, page=page, limit=limit)
    if momvids.can_handle(host): return await momvids.list_videos(base_url=base_url, page=page, limit=limit)
    raise HTTPException(status_code=400, detail="Unsupported host")
...
async def create_scrape(request: Request, body: ScrapeRequestV1) -> ScrapeResponse:
    ...
    if cached_result:
        logging.info(f"âš¡ Cache HIT for scrape {body.url}")
...
async def get_categories(source: str) -> list[CategoryItem]:
    ...
    try:
        ...
        if s in ("blackporntube", "blackporn.tube", "blackpornt"): return [_category_item(c) for c in blackporntube.get_categories()]
        if s in ("sxyland", "sxyland.com"): return [_category_item(c) for c in sxyland.get_categories()]
        if s in ("camcaps", "camcapstv"): return [_category_item(c) for c in camcaps.get_categories()]
        if s in ("koreanpornmovie", "koreanporn", "kpm"): return [_category_item(c) for c in koreanpornmovie.get_categories()]
        if s in ("fullporner", "fullporner.com", "fp"): return [_category_item(c) for c in fullporner.get_categories()]
        if s in ("superporn", "superporn.com"): return [_category_item(c) for c in superporn.get_categories()]
        ...
        if s in ("joysporn", "joysporn.io"): return [_category_item(c) for c in joysporn.get_categories()]
        if s in ("fullxcinema", "fullxcinema.com"): return [_category_item(c) for c in fullxcinema.get_categories()]
        if s in ("filmadult", "film-adult.video"): return [_category_item(c) for c in filmadult.get_categories()]
        if s in ("pornhits", "pornhits.tv"): return [_category_item(c) for c in pornhits.get_categories()]
        if s in ("pornmz", "pornmz.net"): return [_category_item(c) for c in pornmz.get_categories()]
        if s in ("fpoxxx", "fpo.xxx"): return [_category_item(c) for c in fpoxxx.get_categories()]
        if s in ("watchporn", "watchporn.to", "www.watchporn.to"): return [_category_item(c) for c in watchporn.get_categories()]
        if s in ("helloporn", "hello.porn", "www.hello.porn"): return [_category_item(c) for c in helloporn.get_categories()]
        ...
        if s in ("xmilf", "xmilf.com", "www.xmilf.com"): return [_category_item(c) for c in xmilf.get_categories()]
        if s in ("hdzog", "hdzog.com", "www.hdzog.com"): return [_category_item(c) for c in hdzog.get_categories()]
        if s in ("analdin", "analdin.com", "www.analdin.com"): return [_category_item(c) for c in analdin.get_categories()]
        ...
        if s in ("viralchut", "viralchut.com", "www.viralchut.com"): return [_category_item(c) for c in viralchut.get_categories()]
        if s in ("viralmms", "viralmms.com", "www.viralmms.com"): return [_category_item(c) for c in viralmms.get_categories()]
        if s in ("xanimeporn", "xanimeporn.com", "www.xanimeporn.com", "xap"): return [_category_item(c) for c in xanimeporn.get_categories()]
        if s in ("vrsmash", "vrsmash.com", "vrsmashcom"): return [_category_item(c) for c in vrsmash.get_categories()]
        if s in ("porndish", "porndish.com", "www.porndish.com"): return [_category_item(c) for c in porndish.get_categories()]
        if s in ("mypornerleak", "mypornerleak.com", "www.mypornerleak.com"): return [_category_item(c) for c in mypornerleak.get_categories()]
        if s in ("momvids", "momvids.com", "www.momvids.com"): return [_category_item(c) for c in momvids.get_categories()]
